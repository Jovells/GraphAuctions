...
const FUTURE_DATE = Math.floor(new Date(Date.now() + 7 * 24 * 60 * 60).getTime() / 1000)*2;
    ...
    tokenId: BigNumber;
      ...
      describe("createAuction", function () {
        it("should allow a user to create an auction", async function () {
          ...
          const tx = await auction.connect(seller)
            .createAuction(stablecoin.address, startTime, FUTURE_DATE, 10, 'ipgg.com', true, { gasLimit: 3e7 });
      ...
      describe("Bidding", function () {
        ...
        beforeEach(async function () {
          ...
          const block = await ethers.provider.getBlock("latest");
            ...
            const tx = await auction.connect(seller)
            .createAuction(stablecoin.address, startTime, block.timestamp +FUTURE_DATE, 10, "ghh", true, { gasLimit: 3e7 });
        const receipt = await tx.wait();
      ...
      describe("claim & Withdraw", function () {
        ...
        await ethers.provider.send("evm_setNextBlockTimestamp", [endTime+1000]);
          ...
          const tx = await auction.connect(seller)
          .createAuction(stablecoin.address, startTime, endTime+1500, 10, 'ghggh', true, { gasLimit: 3e7 });
            const receipt = await tx.wait();
            auctionId = receipt.events?.find(e=>e.event === 'AuctionCreated')?.args?.[0];
  ...
  describe("Sniping prevention", function(){
    ...
    beforeEach(async function () {
      ...
      const tx = await auction.connect(seller).createAuction(stablecoin.address, startTime, endtime, 10, "ghh", true, { gasLimit: 3e7 });
      const receipt = await tx.wait();
      auctionId = receipt.events?.find(e=>e.event === 'AuctionCreated')?.args?.[0];
    ...
    it("should increase end time when bid is placed with last 5 minutes", async function () {
      ...
      // Approve the auction contract to transfer stablecoin on behalf of bidder1
      await stablecoin.connect(bidder2).approve(auction.address, 100);
